...
class Ion_auth
{
    ...
    public function register($username, $password, $email, $additional_data = array(), $active = FALSE, $notify = FALSE,$pay)
    { //need to test email activation
        ...
        if (!$email_activation || $active == '1') {
            $id = $this->auth_model->register($username, $password, $email, $additional_data, $active,$pay);
            if ($id !== FALSE) {
                if ($notify) {
                    $this->load->library('parser');
                    $parse_data = array(
