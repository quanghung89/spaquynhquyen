...
class Sales extends MY_Controller
{
    ...
    function getSales($warehouse_id = NULL)
    {
        ...
        $edit_link = anchor('products/edit_issue/$1', '<i class="fa fa-edit"></i> ' . lang('edit_sale'), 'class="sledit"');
  ...
  //           <li>' . $return_link . '</li>
        ...
        $this->datatables
            ->select("sales.id, date, reference_no,sma_warehouses.name,CONCAT(sma_users.first_name,' ',sma_users.last_name,' ') as fullname, grand_total")
            ->join('sma_warehouses','sma_warehouses.id = sales.warehouse_id','left')
            ->join('sma_users','sma_users.id = sales.created_by','left')
            ->from('sales');
        ...
        if($user_id){
            $this->datatables->where('sma_sales.created_by',$user_id);
        }

        if($start_date && $end_date){
            $this->db->where($this->db->dbprefix('sales').'.date_1 BETWEEN "' . $start_date . '" and "' . $end_date . '"');
        }else{
            if($start_date){
                $this->db->where($this->db->dbprefix('sales').'.date_1 >=',$start_date);
            }
            if($end_date){
                $this->db->where($this->db->dbprefix('sales').'.date_1 <=',$end_date);
            }
        }
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('category') == 'export_excel' || $this->input->post('category') == 'export_pdf') {
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $sale->name);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $sale->fullname);
