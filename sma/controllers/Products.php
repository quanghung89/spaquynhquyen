...
class Products extends MY_Controller
{
    ...
    function add_issue($quote_id = NULL)
    {
        $this->sma->checkPermissions();
        ...
        if ($this->form_validation->run() == true && $this->sales_model->addSale($data, $products, $payment)) {
            ...
            $this->session->set_flashdata('message', lang("Xuất/Bán thành công"));
            redirect("products");
        } else {
            ...
            $this->page_construct('products/add_issue', $meta, $this->data);
        }
    }

    function edit_issue($id = NULL)
    {
        $this->sma->checkPermissions();
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                'date' => $date,
                'date_1' => $this->sma->fsd(trim($this->input->post('date'))),
                'reference_no' => $reference,
                'customer_id' => $customer_id,
                'customer' => $customer,
                'biller_id' => $biller_id,
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateSale($id, $data, $products)) {
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('sales'), 'page' => lang('sales')), array('link' => '#', 'page' => lang('Sửa Xuất/Bán hàng')));
            $meta = array('page_title' => lang('Sửa Xuất/Bán hàng'), 'bc' => $bc);
            
            $this->page_construct('products/edit_issue', $meta, $this->data);
        }
    }



    function list_purchases($warehouse_id = NULL)
    {
        $this->sma->checkPermissions();
        ...
        $this->page_construct('products/list_purchases', $meta, $this->data);
    ...
    function getProducts($warehouse_id = NULL)
    {
        ...
        if($warehouse_id){
            ...
            . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete1' id='a__$1' href='" . site_url('products/delete_pw/$1').'/'.$warehouse_id . "'>"
       ...
       //      . lang('adjust_quantity') . '</a></li>
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select($this->db->dbprefix('warehouses_products') . ".product_id as productid, " . $this->db->dbprefix('products') . ".image as image, " . $this->db->dbprefix('products') . ".name as name, " . $this->db->dbprefix('products') . ".code as code, " . $this->db->dbprefix('warehouses_products') . ".quantity as quantity, cost as cost, unit, IF(sma_products.date_create_end IS NULL,CONCAT('t_',sma_products.date_create),CONCAT('s_',sma_products.date_create_end)  ) as date, alert_quantity,CONCAT('b_',warehouses_products.disable) as disable", FALSE)
                ...
                if($disable){
                    $this->datatables->where('warehouses_products.disable',$disable);
                }

        } else {
            $this->datatables
                ->select($this->db->dbprefix('products') . ".id as productid, " . $this->db->dbprefix('products') . ".image as image, " . $this->db->dbprefix('products') . ".name as name, " . $this->db->dbprefix('products') . ".code as code, COALESCE(quantity, 0) as quantity , cost as cost, unit, IF(sma_products.date_create_end IS NULL,CONCAT('t_',sma_products.date_create),CONCAT('s_',sma_products.date_create_end)  ) as date,alert_quantity,CONCAT('a_',disable) as disable", FALSE)
                ->from('products')
                ->join('categories', 'products.category_id=categories.id', 'left')
                ->group_by("products.id");
                if($disable){
                    $this->datatables->where('disable',$disable);
                }
        }
        ...
        foreach ($arr->aaData as $key => $value) {
            if(($value[4]*1) > ($value[8]*1)){
               $arr->aaData[$key][4] = '<div style="text-align: center;">Còn ('.$this->sma->formatNumber($value[4]).')</div>';
            }else{
                $arr->aaData[$key][4] = '<div style="color:red; text-align: center;">Hết hàng ('.$this->sma->formatNumber($value[4]).') </div>';
            }
            
            $arrDate = explode('_', $value[7]);
            if($arrDate[0] == 't'){

                $arr->aaData[$key][7] = '<div><span>Tạo ngày: </span>'.$this->sma->hrld($arrDate[1]).'</div>';
            }

            else{
                $arr->aaData[$key][7] = '<div><span style="color:#dea43e">Sửa lần cuối: </span>'.$this->sma->hrld($arrDate[1]).'</div>';
            }


            $arrDis = explode('_', $value[9]);
            if($arrDis[0] == 'a'){
                if($arrDis[1] == 0){
                    $arr->aaData[$key][9] = '<div style="text-align:center"><a class="clickdisable" href="products/change_disable/'.$value[0].'/y'.$arrDis[1].'">Tắt</a></div>';
                }else{
                    $arr->aaData[$key][9] = '<div style="text-align:center"><a class="clickdisable" href="products/change_disable/'.$value[0].'/y'.$arrDis[1].'">Mở</a></div>';
                }
            }else{
                 if($arrDis[1] == 0){
                    $arr->aaData[$key][9] = '<div style="text-align:center"><a class="clickdisable" href="products/change_disable/'.$value[0].'/y'.$arrDis[1].'/'. $warehouse_id .'">Tắt</a></div>';
                }else{
                    $arr->aaData[$key][9] = '<div style="text-align:center"><a class="clickdisable" href="products/change_disable/'.$value[0].'/y'.$arrDis[1].'/'. $warehouse_id .'">Mở</a></div>';
                }
            }
            $arr->aaData[$key][8] = $arr->aaData[$key][9];
            $arr->aaData[$key][9] = $arr->aaData[$key][10];
            // unset($arr->aaData[$key][9]);
            unset($arr->aaData[$key][10]);

        }

        // $this->datatables->unset_column('alert_quantity');
    ...
    function add_receipt(){
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array('reference_no' => $reference,
                'date' => $date,
                'date_1' => $this->sma->fsd($this->sma->hrsd($date)),
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPurchase($data, $products)) {
            ...
            $this->page_construct('products/add_receipt', $meta, $this->data);
        }
    }

    function edit_receipt($id = NULL)
    {
        $this->sma->checkPermissions();
        ...
        if ($this->form_validation->run() == true) {
            ...
            if ($date) {
                $data['date'] = $date;
                $data['date_1'] = $this->sma->fsd($this->sma->hrsd($date));
            }
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updatePurchase($id, $data, $products)) {
            ...
            $this->session->set_userdata('remove_pols', 1);
            $this->data['csrf'] = $this->session->userdata('user_csrf');
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('purchases'), 'page' => lang('Sửa nhập hàng')), array('link' => '#', 'page' => lang('Sửa nhập hàng')));
            $meta = array('page_title' => lang('Sửa nhập hàng'), 'bc' => $bc);
            $this->page_construct('products/edit_receipt', $meta, $this->data);
        }
    }
    ...
    function change_disable($id,$disable,$warehouses){
        $success = false;

        if($this->products_model->change_disable($id,$disable,$warehouses)){
            $success = true;
            $msg = 'Thành công';
        }else{
            $msg = 'Thất bại';
    ...
    function add($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            $data = array(
                ...
                'price' => str_replace(",","",$this->input->post('cost')),
                'alert_quantity' => 10,
                'quantity' => 0,
    ...
    function check_error(){
        ...
        $val = $this->input->post('valcode');
        $flag = true;
        
        $all = $this->products_model->getAllProducts();
        ...
        if($id){
            $this1 = $this->products_model->getProductByID($id);
                if($this1->code == $val){                                          
                    $flag =  true;
                }
        }
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            $data = array(
                ...
                'cost' => str_replace(",","",$this->input->post('cost')),
                'unit' => $this->input->post('unit'),
                'date_create_end' => date('Y-m-d H:i:s'),
                'price' => str_replace(",","",$this->input->post('cost')),
    ...
    function delete($id = NULL)
    {
        ...
        if($this->products_model->getCountPurch($id) > 0 || $this->products_model->getCountSale($id) > 0){
            $this->sma->checkPermissionsModelView(false,true,'','Xóa thất bại vì đã nhập hàng hoặc xuất hàng');
        }
     ...
     function delete_pw($id = NULL, $warehouse_id = null)
    {
        $this->sma->checkPermissions(NULL, TRUE);
        ...
        if($this->products_model->getCountPurch($id,$warehouse_id) > 0 || $this->products_model->getCountSale($id, $warehouse_id) > 0){
            $this->sma->checkPermissionsModelView(false,true,'','Xóa thất bại vì đã nhập hàng hoặc xuất hàng');
        }

        if ($this->products_model->deleteProductWH($id,$warehouse_id)) {
            if($this->input->is_ajax_request()) {
                echo lang("product_deleted"); die();
            }
            $this->session->set_flashdata('message', lang('product_deleted'));
    ...
    function product_actions($warehouse_id)
    {
        if (!$this->Owner) {
            ...
            if (!empty($this->input->post('val'))) {
                ...
                if ($this->input->post('category') == 'delete') {
                    ...
                    foreach ($this->input->post('val') as $id) {
                        ...
                        if($warehouse_id){
                            if($this->products_model->getCountPurch($id,$warehouse_id) > 0 || $this->products_model->getCountSale($id,$warehouse_id) > 0){
                                $msg[$i] = '<br>Xóa thất bại sản phẩm '. strtoupper($product->name) .' vì đã nhập hàng và xuất hàng';
                            }else{
                                if($this->products_model->deleteProductWH($id,$warehouse_id)){
                                    $msg[$i] = '<br>Xóa thành công sản phẩm '. strtoupper($product->name);
                                }else{
                                    $msg[$i] = '<br>Xóa thất bại sản phẩm '. strtoupper($product->name);
                                }
                            }
                            ...
                            if($this->products_model->getCountPurch($id) > 0 || $this->products_model->getCountSale($id) > 0){
                                $msg[$i] = '<br>Xóa thất bại '. strtoupper($product->name) .' vì đã nhập hàng và xuất hàng';
                            }else{
                                if($this->products_model->deleteProduct($id)){
                                    $msg[$i] = '<br>Xóa thành công sản phẩm '. strtoupper($product->name);
