...
class system_settings extends MY_Controller
{
    ...
    // function list_products($warehouse_id = NULL)
    // {
    ...
    //     $this->page_construct('settings/list_products', $meta, $this->data);
    // }
    ...
    function index()
    {
        ...
        $this->form_validation->set_rules('time_notice', lang('Thời gian hiện thông báo'), 'trim|required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array('site_name' => DEMO ? 'Stock Manager Advance' : $this->input->post('site_name'),
                ...
                'time_notice' => $this->input->post('time_notice'),
    ...
    function create_group()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            $data = array('billler_active' => $this->input->post('billler_active'),'name' => strtolower($this->input->post('group_name')), 'description' => $this->input->post('description'));
            $new_group_id = $this->settings_model->addGroup($data);
            if ($new_group_id) {
                $this->session->set_flashdata('message', lang('Thêm thành công'));
    ...
    function edit_group($id)
    {
        ...
        if ($this->form_validation->run() === TRUE) {
            $data = array(
                'billler_active' => $this->input->post('billler_active'),
    ...
    function time_work()
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('Giờ làm')));
        $meta = array('page_title' => lang('Giờ làm'), 'bc' => $bc);
        $this->page_construct('settings/time_work', $meta, $this->data);
    }

    function getCategories()
    {

        // <a href='" . site_url('system_settings/subcategories/$1') . "' class='tip' title='" . lang("list_subcategories") . "'><i class=\"fa fa-list\"></i></a>
        $this->load->library('datatables');
        $this->datatables
            ->select("sma_categories.id, sma_categories.code, sma_categories.name, sma_categories.name_parent, sma_categories.time,sma_categories.price,
                  GROUP_CONCAT(sma_category_assign.sma_category_assign_userid SEPARATOR ',') as inamestaffass ")
            ->from("sma_categories")
            ->join("sma_category_assign","sma_categories.id = sma_category_assign.sma_category_assign_categoryid","left")
            ->group_by('sma_categories.id')
            ->add_column("Actions", "<center> <a href='" . site_url('system_settings/edit_category/$1') . "' data-toggle='modal' data-target='#myModal' class='tip' title='" . lang("edit_category") . "'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_category") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('system_settings/delete_category/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></center>", "sma_categories.id");

        $arr = json_decode($this->datatables->generate());
        foreach ($arr->aaData as $key => $value) {            

            $arrStaffA = explode(',', $value[6]);
            ...
            $arr->aaData[$key][6] = implode(', ', $arrStaffName);
    ...
    function getTimework()
    {
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select("id, sma_timework_code, sma_timework_name,sma_timework_starttime,sma_timework_endtime")
            ->from("sma_timework")
            ->add_column("Actions", "<center> <a href='" . site_url('system_settings/edit_timework/$1') . "' data-toggle='modal' data-target='#myModal' class='tip' title='" . lang("Sửa giờ làm") . "'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("Xóa giờ làm") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('system_settings/delete_timework/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></center>", "id");
        $arr = json_decode($this->datatables->generate());
        foreach ($arr->aaData as $key => $value) {
            $arr->aaData[$key][3] = date('H:i:s',$value[3]);
            $arr->aaData[$key][4] = date('H:i:s',$value[4]);
        }
        echo json_encode($arr);
    }
     ...
     function add_timework()
    {
        ...
        if ($this->form_validation->run() == true) {
            $data = array(
                'sma_timework_code' => $this->input->post('code'),
                'sma_timework_name' => $this->input->post('name'),
                'sma_timework_starttime' => strtotime($this->input->post('startime')),
                'sma_timework_endtime' => strtotime($this->input->post('endtime')),
        ...
        if ($this->form_validation->run() == true) {
            $success = false;
            $msg = "Thêm thất bại";
            if($this->settings_model->addTimeWork($data)){
                $success = true;
                $msg = "Thêm thành công";
            }
            ...
            $this->data['name'] = array('name' => 'name',
             ...
             $this->data['price'] = array('name' => 'price',
                'id' => 'price',
                'type' => 'text',
                'class' => 'form-control formatMoney',
                'required' => 'required',
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/add_timework', $this->data);
        }
    }
    ...
    function add_category()
    {
        ...
        if ($this->form_validation->run() == true && $this->input->is_ajax_request()) {
            ...
            $category_time = $this->input->post('category_time');
            if($this->site->getCategoryByID($parent_id)){
                $parent_name = $this->site->getCategoryByID($parent_id)->name;
            }
        ...
        if ($this->form_validation->run() == true) {
            $success = false;
            $msg = "Thêm thất bại";
            if($this->settings_model->addCategory($data,$assign)){
                $success = true;
                $msg = "Thêm thành công";
            }
            echo json_encode(array(
    ...
    function edit_timework($id = NULL)
    {
        $this->load->helper('security');
        $this->form_validation->set_rules('code', lang("category_code"), 'trim|required');
        $inv = $this->settings_model->getTimeWorkByID($id);   
        ...
        if ($this->form_validation->run() == true) {
            $success = false;
            $msg = "Sửa thất bại";
            if($this->settings_model->updateTimeWork($id,$data)){
                $success = true;
                $msg = "Sửa thành công";
            }
            echo json_encode(array(
            ...
            $this->data['name'] = array('name' => 'name',
                ...
                'class' => 'form-control',
                'required' => 'required',
                'value' => $this->form_validation->set_value('name', $inv->sma_timework_name),
            );
            $this->data['code'] = array('name' => 'code',
                'id' => 'code',
                'type' => 'text',
                'class' => 'form-control',
                'required' => 'required',
                'value' => $this->form_validation->set_value('code', $inv->sma_timework_code),
            );
            $this->data['inv'] = $inv;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['id'] = $id;
            $this->load->view($this->theme . 'settings/edit_timework', $this->data);
        }
    }
    ...
    function edit_category($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->input->is_ajax_request()) {
            $data = array(
                ...
                'name' => $this->input->post('name'),
                'price' => str_replace(',', '', $this->input->post('price')) ,
                'id_parent' => $this->input->post('category_parent'),
                'time' => $this->input->post('category_time'),
            );
            if($this->site->getCategoryByID($this->input->post('category_parent'))){
                $parent_name = $this->site->getCategoryByID($this->input->post('category_parent'))->name;
            }

            $data['name_parent'] = $parent_name ;
            $assign = $this->input->post('assign');
            if ($_FILES['userfile']['size'] > 0) {
                $this->load->library('upload');
                $config['upload_path'] = $this->upload_path;
        ...
        if ($this->form_validation->run() == true && $this->input->is_ajax_request()) {
            ...
            $success = false;
            $msg = "Sửa thất bại";
            if($this->settings_model->updateCategory($id, $data, $photo,$assign)){
                $success = true;
                $msg = "Sửa thành công";
            }
            echo json_encode(array(
            ...
            $cate = $this->site->getAllCategories1(null);
            foreach ($cate as $key => $value) {
                if($value->id_parent){
                    $cate[$key]->name_parent = $this->site->getCategoryByID($value->id_parent)->name;
                } 
            ...
            $this->data['categories'] = $cate;
            $arrAssign = $this->site->getCategoryAsgin($id);
    ...
    function delete_timework($id = NULL)
    {
        if ($this->settings_model->deleteTimework($id)) {
            echo lang("Xóa thành công");
        }
    }
