...
class Purchases extends MY_Controller
{
    ...
    function getPurchases($warehouse_id = NULL)
    {
        ...
        if($this->input->get('start_date')){
            $start_date = $this->sma->fsd($this->input->get('start_date'));
        }

        if($this->input->get('end_date')){
            $end_date = $this->sma->fsd($this->input->get('end_date'));
        }
        if($warehouse_id == 'all'){
            $warehouse_id = '';
        }
        ...
        $edit_link = anchor('products/edit_receipt/$1', '<i class="fa fa-edit"></i> ' . lang('edit_purchase'));
        ...
        $this->datatables
            ->select("purchases.id, date, reference_no, sma_warehouses.name, CONCAT(sma_users.first_name,' ' ,sma_users.last_name,'') as fullname ,grand_total")
            ->join('sma_warehouses','sma_warehouses.id = sma_purchases.warehouse_id','left')
            ->join('sma_users','sma_users.id = sma_purchases.created_by','left')
            ->from('purchases');

        if($warehouse_id){
            $this->datatables->where('sma_purchases.warehouse_id',$warehouse_id);
        }

        if($user_id){
            $this->datatables->where('sma_purchases.created_by',$user_id);
        }

        if($start_date && $end_date){
            $this->db->where($this->db->dbprefix('purchases').'.date_1 BETWEEN "' . $start_date . '" and "' . $end_date . '"');
        }else{
            if($start_date){
                $this->db->where($this->db->dbprefix('purchases').'.date_1 >=',$start_date);
            }
            if($end_date){
                $this->db->where($this->db->dbprefix('purchases').'.date_1 <=',$end_date);
            }
        }
...
//generate pdf and force to download
    ...
    function add($quote_id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPurchase($data, $products)) {
            ...
            $this->session->set_flashdata('message', $this->lang->line("Thêm thành công"));
            if($this->Owner || $this->Admin){
                redirect('purchases');
            }else{
                redirect('products');
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('category') == 'export_excel' || $this->input->post('category') == 'export_pdf') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('D1', lang('Người tạo'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $purchase->name);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $purchase->fullname);
