...
class Settings_model extends CI_Model
{
    ...
    {
        $q = $this->db->get_where("sma_timework", array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function addTimeWork($data){

        if ($this->db->insert("sma_timework", $data )) {
            return true;
        }
        return false;
    }

    public function addCategory($data,$assign)
    {
        if ($this->db->insert("categories", $data)) {
            $idc = $this->db->insert_id();
            $count = 0;
            foreach ($assign as $key => $value) {
                $item['sma_category_assign_categoryid'] = $idc;
                $item['sma_category_assign_userid'] = $value;
                if( $this->db->insert('sma_category_assign',$item) ){
                    $count++;
                }
            }        
    ...
    public function updateCategory($id, $data = array(), $photo,$assigns)
    {
        ...
        if ($this->db->update("categories", $categoryData)) {
            ...
            foreach ($assigns as $key => $value) {
                ...
                $inv = $this->site->getCategoryAsgin($id,$value);
                if($inv){
                    $idA[] = $inv[0]['sma_category_assign_id'];
                }else{
                    $data_new = array(
                        'sma_category_assign_categoryid' => $id,
                        'sma_category_assign_userid' => $value,
                    );
                    if($this->db->insert('sma_category_assign',$data_new)){
                        $idA[] = $this->db->insert_id();
                    }
                } 
            }

            $this->db->where_not_in('sma_category_assign.sma_category_assign_id',$idA);
            $this->db->where('sma_category_assign.sma_category_assign_categoryid',$id);
            $this->db->delete('sma_category_assign');
    ...
    public function updateTimeWork($id, $data = array())
    {
        $this->db->where('id', $id);
        if ($this->db->update("sma_timework", $data)) {
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->db->delete("sma_timework", array('id' => $id))) {
            return true;
        }
        return FALSE;
    }
