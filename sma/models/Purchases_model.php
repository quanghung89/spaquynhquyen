...
class Purchases_model extends CI_Model
{
    ...
    {
        $this->db->select('sma_purchases.*, CONCAT(sma_users.first_name," ",sma_users.last_name,"") as fullname, sma_warehouses.name');
        $this->db->join('sma_users','sma_users.id = sma_purchases.created_by','left');
        $this->db->join('sma_warehouses','sma_warehouses.id = sma_purchases.warehouse_id','left');
        $q = $this->db->get_where('sma_purchases', array('sma_purchases.id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
