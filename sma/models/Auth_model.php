...
class Auth_model extends CI_Model
{
    ...
    public function getTimeoutDESC($id){
        $this->db->order_by('sma_timeout.sma_timeout_id DESC');
        $q = $this->db->get_where('sma_timeout', array('sma_timeout.sma_timeout_userid' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function deleteBouns($id){
        if($this->db->delete('sma_bouns',array('sma_bouns.sma_bouns_id' => $id)) ) {
            return true;
        }
        return false;
    }
    ...
    public function deleteFine($id){
        if($this->db->delete('sma_fine',array('sma_fine.sma_fine_id' => $id)) ) {
            return true;
        }
        return false;
    }

    public function deleteOtex($id){
        if($this->db->delete('sma_otex',array('sma_otex.sma_otex_id' => $id)) ) {
            return true;
        }
        return false;
    }
    ...
    public function deletePay($data){
        if($this->db->delete('sma_pay',array('sma_pay.sma_pay_id' => $data['now']['sma_pay_id'])) &&  $this->db->update('sma_pay', array('sma_pay.sma_pay_enddate' => NULL), array('sma_pay.sma_pay_id' => $data['prev']['sma_pay_id'])) ){
            return true;
        }
        return false;
    }

    public function updateBouns($data,$id){
        if($id){
            if($this->db->update('sma_bouns',$data,array('sma_bouns_id'=>$id))){
                return true;
            }
        }else{
            if($this->db->insert('sma_bouns',$data)){
                return true;
            }
        }
        return false;
    ...
    public function updateOxte($data,$id){
        if($id){
            if($this->db->update('sma_otex',$data,array('sma_otex_id'=>$id))){
                return true;
            }
        }else{
            if($this->db->insert('sma_otex',$data)){
                return true;
            }
        }
        return false;
    ...
    public function updatePay($data,$id){
        $inv = $this->site->getPayByIDUser($data['sma_pay_userid'],1,'DESC');
        if($id){
            if($this->db->update('sma_pay',$data,array('sma_pay_id'=>$id))){
                return true;
            }
        }else{
            if($this->db->insert('sma_pay',$data)){
                if($this->db->update('sma_pay',array('sma_pay_enddate'=>$data['sma_pay_startdate']),array('sma_pay_id'=>$inv[0]['sma_pay_id'])) ){
                   return true; 
                }
            }
        }
     ...
     public function updateFine($data,$id){
        if($id){
            if($this->db->update('sma_fine',$data,array('sma_fine_id'=>$id))){
                return true;
            }
        }else{
            if($this->db->insert('sma_fine',$data)){
                return true;
            }
        }
        return false;
    ...
    public function delete_timeout($id){
        if($this->db->delete('sma_timeout',array('sma_timeout_id'=>$id))){
            return true;
        }
        return false;
    }

    public function updateTimeout($id,$data){
        if(!$this->getTimeoutDESC($id)){
            if($this->db->insert('sma_timeout',$data)){
                return true;
            }
        }else{
            if($this->getTimeoutDESC($id)->sma_timeout_enddate <= $data['sma_timeout_startdate']){
                if($this->db->insert('sma_timeout',$data)){
                    return true;
                }
            }
    ...
    public function updateTimework($id,$data){
        $count = 0;
        if($this->db->delete('sma_usertimework',array('sma_usertimework_userid'=>$id))){
            foreach ($data as $key => $value) {
                if($this->db->insert('sma_usertimework',$value)){
                    $count++;
                }
            }
    ...
    public function register($username, $password, $email, $additional_data = array(), $active = FALSE, $pay)
    {
