...
class Sales_model extends CI_Model
{
    ...
    {   
        $this->db->select('sma_sales.*, CONCAT(sma_users.first_name," ",sma_users.last_name,"") as fullname, sma_warehouses.name');
        ...
        $q = $this->db->get_where('sma_sales', array('sma_sales.id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
