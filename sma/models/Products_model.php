...
class Products_model extends CI_Model
{
    ...
    public function getQProductWa($id,$disable=NULL){
        ...
        return $this->db->count_all_results('sma_warehouses_products');

    }

    public function getCountPurch($id,$warehouses=NULL){       
        if($warehouses){            
            $this->db->where('warehouses_id',$warehouses);
        }
        $this->db->where('purchase_id <>',NULL);
        $this->db->where('product_id',$id);
        return $this->db->count_all_results('sma_purchase_items');

    }

    public function getCountSale($id,$warehouses=NULL){       
        if($warehouses){            
            $this->db->where('warehouses_id',$warehouses);
        }
        $this->db->where('product_id',$id);
        return $this->db->count_all_results('sma_sale_items');

    }

    public function change_disable($id,$disable,$warehouses){        
        if($warehouses){
            if($disable == 'y0'){  
                if($this->db->update('sma_warehouses_products',array('disable'=>1),array('warehouse_id'=>$warehouses,'product_id'=>$id)) ){
                    $d = $this->getQProductWa($id,2);
                    if($d == 0){                        
                        $this->db->update('sma_products',array('disable'=>1),array('id'=>$id));
                    }  
                    return true;                 
                }      
            }else{  
                if($this->db->update('sma_warehouses_products',array('disable'=>0),array('warehouse_id'=>$warehouses,'product_id'=>$id)) ){
                    $d = $this->getQProductWa($id,1);
                    $al = $this->getQProductWa($id);   
                    if($d == 0){
                       
                        $this->db->update('sma_products',array('disable'=>0),array('id'=>$id));
                    }  
                    return true;                 
                }      
            }  
            ...
            if($disable == 'y0'){
                if($this->db->update('sma_products',array('disable'=>1),array('id'=>$id)) &&  $this->db->update('sma_warehouses_products',array('disable'=>1),array('product_id'=>$id)) ){
                    return true;
                }      
            }else{
                if($this->db->update('sma_products',array('disable'=>0),array('id'=>$id)) &&  $this->db->update('sma_warehouses_products',array('disable'=>0),array('product_id'=>$id)) ){
                    return true;
                } 
            }      
        }
    ...
    public function getQuantityProductWH($id,$warehouse_id){
        $this->db->select_sum('sma_warehouses_products.quantity');
        $q = $this->db->get_where('sma_warehouses_products', array('sma_warehouses_products.product_id' => $id,'sma_warehouses_products.warehouse_id'=>$warehouse_id));
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function getQuantityPur($id,$warehouse_id){
        $this->db->select('sma_purchase_items.quantity_balance');
        $this->db->order_by('sma_purchase_items.id ASC');
        $q = $this->db->get_where('sma_purchase_items', array('sma_purchase_items.product_id' => $id,'sma_purchase_items.warehouse_id'=>$warehouse_id),1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function deleteProductWH($id,$warehouse_id)
    {
        $quantityPWH = $this->getQuantityProductWH($id, $warehouse_id);
        $product = $this->getProductByID($id);
        $quantityP = $product->quantity * 1;
        $purchases = $this->getQuantityPur($id,$warehouse_id);
        $quantityPur = $product->quantity_balance * 1;
        if ($this->db->update('sma_products', array('quantity'=> $quantityP - $quantityPWH) ,array('id' => $id)) 
            && $this->db->delete('sma_warehouses_products', array('product_id' => $id,'warehouse_id' => $warehouse_id)) 
            && $this->db->update('sma_purchase_items', array('quantity_balance'=> $quantityPur - $quantityPWH) ,array('id' => $purchases->id))  ) {
            return true;
        }
        return FALSE;
    }
